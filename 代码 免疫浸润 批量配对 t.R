...
#整理数据
paired_data <- as.data.frame(t(norm_ssGSEA_Score))
...
bb.sample <- colnames(bb)[1:73]#去掉 group 和 paired 那两列
test.b <- c()
x <- c("a","b")
y <- c("a","a")
...
colnames(test.b) <- colnames(bb)[1:73]
rownames(test.b) <- levels(bb$Group)
test.b <- melt(test.b)
colnames(test.b) <- c("Group","variable","value")

library(tidyverse)
test.b1 <- bb[,c(1,165)] %>% gather(variable,value,Group) %>% group_by(variable,Group) %>% 
  summarise(Max = max(value))
test.b11 <- dcast(test.b1,Group~variable)
for (i in 2:ncol(test.b11)) {
  test.b11[,i] <- test.b11[,i] + max(test.b11[,i])*0.015}
test.b11 <- melt(test.b11)
test.b1 <- merge(test.b1,test.b11,by = c("variable","Group"))
test.b2 <- merge(test.b,test.b1,by = c("variable","Group"))

bb1$variable <- factor(bb1$variable,levels = c("Regulatory_T_1","Regulatory_T_2","Regulatory_T_3","Regulatory_T_4"))
test.b2$variable <- factor(test.b2$variable,levels = c("Regulatory_T_1","Regulatory_T_2","Regulatory_T_3","Regulatory_T_4"))
...
print(p)
png("alpha_diversity_5.png",width = 4000,height = 3600,res = 600)

dev.off()
pdf("alpha_diversity_1.pdf",width = 7,height = 6)
p
dev.off()

#immunoai----

immunoai=read.table('/Users/louliming/Desktop/原发复发数据/03评价免疫微环境/TIMER\ 0703\ 重新算/0703\ timer.txt',header = T,sep = '\t')  #'data.txt'是文件名
immunoai <- as.data.frame(immunoai)
rownames(immunoai)=immunoai[,1]
immunoai=immunoai[,-1]

paired_data <- as.data.frame(immunoai)
...
paired_data1  <- rownames_to_column(paired_data, var = "ID")
write.table(bb,file=' Timer免疫浸润 转置.txt',quote = F,sep = '\t',row.names = T)

bb <- as.data.frame(paired_data)#对列的名字非常的敏感，不能有空格,-,+等奇怪符号
bb <- bb %>%
  mutate_at(vars(1:107), as.numeric)
...
bb.sample <- colnames(bb)[1:107]# 要改！！！！去掉 group 和 paired 那两列，要不然会出现报错 Error in .subset2(x, i, exact = exact) : attempt to select less than one element in integerOneIndex
test.b <- c()
x <- c("a","b")
y <- c("a","a")
...
colnames(test.b) <- colnames(bb)[1:107]
rownames(test.b) <- levels(bb$Group)
test.b <- melt(test.b)
colnames(test.b) <- c("Group","variable","value")
